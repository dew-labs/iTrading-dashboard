...
const Input = forwardRef<HTMLInputElement, InputProps>(({
  ...
  const getIconPosition = (position: 'left' | 'right') => {
    ...
    const horizontalPosition = position === 'left'
      ? inputSize === 'sm' ? 'left-2.5' : inputSize === 'lg' ? 'left-3.5' : 'left-3'
      : inputSize === 'sm' ? 'right-2.5' : inputSize === 'lg' ? 'right-3.5' : 'right-3'
  ...
  return (
    <div className={cn('space-y-1', !fullWidth && 'inline-block')}>
      ...
      <div className='relative'>
        {/* Left Icon */}
        {LeftIcon && (
          <LeftIcon className={cn(getIconClasses(), getIconPosition('left'))} />
        )}
        ...
        {/* Right Icon */}
        {RightIcon && (
          <RightIcon className={cn(getIconClasses(), getIconPosition('right'))} />
        )}
      </div>
